#!/usr/bin/env python3
"""
ANIMATED REACTION BOT – PUBLIC /bots, ALL TOKENS USED
"""
...
def send_message(chat_id: int, text: str, reply_markup=None, photo_url=None, animation_url=None):
    if photo_url:
        ...
        return api_request(BOT_TOKENS[0], "sendPhoto", data)
    elif animation_url:
        data = {"chat_id": chat_id, "animation": animation_url, "caption": text, "parse_mode": "HTML"}
        if reply_markup:
            data["reply_markup"] = reply_markup
        return api_request(BOT_TOKENS[0], "sendAnimation", data)
    else:
...
def send_reaction(token: str, chat_id: int, msg_id: int, emoji: str, is_big: bool = True):
    data = {
        "chat_id": chat_id,
        "message_id": msg_id,
...
def send_multiple_reactions(chat_id: int, msg_id: int, msg_type: str = "text"):
    ...
    print(f"🎯 Adding {num} reactions to msg {msg_id} (using all {len(available)} tokens)")
    ...
    for i in range(num):
        ...
        t = threading.Thread(
            target=lambda tkn=token, e=emoji, big=is_big: (
                send_reaction(tkn, chat_id, msg_id, e, big) and db.increment_reactions()
            ),
            daemon=True
        )
        threads.append(t)
...
def handle_command(command: str, chat_id: int, user_id: int, username: str = ""):
    if command == '/start':
        ...
        # Force subscription check (except owner)
...
👉 Click <b>Help</b> below to learn how to use me."""

        # Send with GIF animation if available
        if WELCOME_GIF_URL:
...
1. Use /bots to see all bot usernames
2. Add each bot as admin in your channel/group
3. Enable <b>Add Reactions</b> permission
4. Post any message – I will react automatically
...
/bots – List all bot usernames (public)
...
<b>How it works:</b>
• Each bot token adds one reaction
• First 3 reactions are <b>BIG & ANIMATED</b>
• Works in channels, groups, private chats

<b>Setup:</b>
1. Use /bots to see all bot usernames
2. Add each bot as admin with "Add Reactions" permission
3. Post any message – I will react

<b>Commands:</b>
/start – Menu
/help – This
/stats – Statistics (owner)
/lock /unlock – Control (owner)
/bots – List bot usernames (public)
